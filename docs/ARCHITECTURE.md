...
|---|------|----------|---------|
| D1 | `FinanzasContext` | God-object: dominio + UI + crypto + PDF + IA + sync mezclados. **En progreso**: extraído `convertMarkdownToHtml`→`utils/` y `buildChatPdfHtml`→`services/chatPdfExport.ts` (función pura del export PDF; el contexto queda como wrapper delgado iframe/print) → 2572→1922 líneas. Pendiente: hooks `useAiAdvisor`/`useSecurity`/`useBackupSync`/`useTransactions`/`useDebts`/`useAccounts` | Altísimo coste de cambio |
