...
export const buildFinanceDataPrompt = (params: PromptContextParams): string => {
	...
	const transactionsText =
		params.filteredTransactions.length > 0
			? params.filteredTransactions
					.map(
						(t) =>
							`- Concepto: "${t.desc}", Importe: ${t.money ? toNumber(t.money.amount).toFixed(2) : '0.00'}€, Tipo: ${t.type}, Frecuencia: ${t.recurrence === 'recurring' ? 'Recurrente' : 'Puntual/Único'}, Etiqueta/Categoría actual: "${t.tag}", Propietario: ${t.owner}`
					)
					.join('\n')
	...
	const describeDebtForPrompt = (debt: Debt) => {
		const ownerLabel =
			debt.owner === 'userA' ? params.userAName : debt.owner === 'userB' ? params.userBName : 'Conjunta';
					...
					.map((d) => {
						...
						const status = isActive
							...
							: isFuture
								...
								: 'Finalizada o inactiva en este mes';
	...
	const consolidationText =
		params.consolidatedDebtsObjects.length > 0
			? `
        ...
        - Propuesta nueva reunificada: Plazo de ${params.consolidationFormTermMonths} meses con un TAE/CER de ${params.consolidationFormTae}%.
