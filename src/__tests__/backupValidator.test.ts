...
describe('Validador de Copias de Seguridad (backupValidator)', () => {
	...
	describe('Validación de Copia de Seguridad Completa', () => {
		...
		it('debe fallar si no contiene información financiera', () => {
			...
			expect(() => validateAndSanitizeBackup({ finanzas_v3_userA_name: 'Ana' })).toThrow('no contiene información financiera');
		});

		it('debe fallar si cuentas, transacciones, deudas o periodos no son arrays', () => {
			expect(() => validateAndSanitizeBackup({ ...validBackup, finanzas_v3_accounts: {} })).toThrow("debe ser un array");
			expect(() => validateAndSanitizeBackup({ ...validBackup, finanzas_v3_transactions: 123 })).toThrow("debe ser un array");
			expect(() => validateAndSanitizeBackup({ ...validBackup, finanzas_v3_debts: {} })).toThrow("debe ser un array");
			expect(() => validateAndSanitizeBackup({ ...validBackup, finanzas_v3_periods: true })).toThrow("debe ser un array");
			expect(() => validateAndSanitizeBackup({ ...validBackup, finanzas_v3_ai_chat: {} })).toThrow("debe ser un array");
		});
		...
		it('debe fallar con tipo de transacción o pago inválidos', () => {
			expect(() => validateAndSanitizeBackup({
				...validBackup,
		...
		it('debe fallar si una deuda fraccionada tiene cuotas no válidas', () => {
			...
			expect(() => validateAndSanitizeBackup({
				...
				finanzas_v3_debts: [{
					...
					fees: 15,
					totalToPay: 315,
					installments: [{ id: 'inst-1', dueMonth: '2026/06', amount: 105, status: 'paid', label: 'Cuota' }]
				}]
			...
			expect(() => validateAndSanitizeBackup({
				...
				finanzas_v3_debts: [{
					...
					fees: 15,
					totalToPay: 315,
					installments: [{ id: 'inst-1', dueMonth: '2026-06', amount: 105, status: 'invalid-status', label: 'Cuota' }]
				}]
		...
		it('debe fallar si los periodos o el chat de IA contienen datos erróneos', () => {
			expect(() => validateAndSanitizeBackup({
				...validBackup,
				finanzas_v3_periods: [{ month: '2026/05', openingBalance: 100, openingBalanceA: 50, openingBalanceB: 50 }]
